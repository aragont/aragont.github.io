...
1. Создать файл A.
2. Скопировать созданный файл A через нашу программу в файл B, сделав его разреженным:
`$ ./myprogram fileA fileB`
3. Сжать A и B с помощью `gzip`
4. Распаковать сжатый файл B в stdout и сохранить через программу в файл C:
...
Файл шаблона для слежения за потомками [здесь](fork_waitpid.c). Шаблон для создания демона на основе примера из книги Робачевского [здесь](demonize.c).
